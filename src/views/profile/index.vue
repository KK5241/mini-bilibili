          ...
          <div v-else-if="activeTab === 'history'" class="p-4">
            ...
            <div v-else class="space-y-4">
              <div
                ...
                @click="goToVideo(video.video.id)"
              >
                ...
                <div class="w-4/5 pl-4">
                  ...
                  <div class="flex items-center text-xs text-gray-500">
                    ...
                    <div>
                      ...
                      {{
                        formatDate(
                          video.video.viewedAt || video.video.createdAt,
                        )
                      }}
                    </div>
                  </div>
            ...
            >
              ...
              <el-form-item label="头像">
                <div class="flex items-center">
                  <div
                    class="relative group w-16 h-16 rounded-full overflow-hidden"
                  >
                    <img
                      :src="
                        editForm.avatarPreview ||
                        (editForm.avatar
                          ? getCompleteFileUrl(editForm.avatar)
                          : '/src/assets/avatar-default.png')
                      "
...
const editRules = {
  ...
  bio: [{ max: 200, message: '个人简介最多200个字符', trigger: 'blur' }],
}
  ...
  try {
    ...
    profileData.value = {
      ...
      avatar: profileResponse.avatar
        ? getCompleteFileUrl(profileResponse.avatar)
        : '',
    }
    ...
    if (userStore.isLoggedIn && !isCurrentUser.value) {
      ...
      isFollowed.value =
        ...
        following.some((user: UserItem) => user.id === userId.value)
    }
  ...
  try {
    switch (tab) {
      case 'videos':
        ...
        userVideos.value = (videosRes as unknown as VideoItem[]).map(
          (video) => ({
            ...video,
      ...
      case 'favorites':
        ...
        favoriteVideos.value = (favoritesRes as unknown as VideoItem[]).map(
          (video) => ({
            ...video,
            cover: video.cover ? getCompleteFileUrl(video.cover) : '',
          }),
      ...
      case 'history':
        if (isCurrentUser.value) {
          ...
          historyVideos.value = (historyRes as unknown as HistoryItem[]).map(
            (item) => ({
              ...item,
              video: {
                ...item.video,
                cover: item.video.cover
                  ? getCompleteFileUrl(item.video.cover)
                  : '',
              },
              user: item.user
                ? {
                    ...
                    avatar: item.user.avatar
                      ? getCompleteFileUrl(item.user.avatar)
                      : '',
                  }
                : undefined,
            }),
        ...
        console.log('123', historyVideos.value)
        break
    }
  } catch (error) {
...
const showFollowDialog = async (type: 'following' | 'followers') => {
  ...
  try {
    ...
    if (userStore.isLoggedIn) {
      ...
      const followingIds = (myFollowing as unknown as UserItem[]).map(
        (user: UserItem) => user.id,
      )
      ...
      try {
        ...
        if (result && typeof result === 'object') {
          ...
          const avatarUrl = userData.avatar
            ? getCompleteFileUrl(userData.avatar)
            : editForm.avatar
              ? getCompleteFileUrl(editForm.avatar)
          ...
          // 处理头像URL，确保是完整URL
          const avatarUrl = editForm.avatar
            ? getCompleteFileUrl(editForm.avatar)
            : profileData.value.avatar
